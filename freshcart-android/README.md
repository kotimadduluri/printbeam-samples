...
| Auto-naming | [`SettingsViewModel.kt`](app/src/main/java/com/freshcart/ui/settings/SettingsViewModel.kt) (`resolveNameFromDevice`) | `PrintBeam.queryDeviceInfo(id)` — since alpha03 the SDK names network printers in the scan list itself (`GS I` during discovery); this is the fallback for a printer picked or entered nameless: ask for its identity over the held session, then re-register the resolved name |
